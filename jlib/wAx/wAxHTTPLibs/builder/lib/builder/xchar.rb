...
# If the WAx::WAxHTTPLibs::Builder::XChar module is not currently defined, fail on any
...
if ! defined?(WAx::WAxHTTPLibs::Builder::XChar) and ! String.method_defined?(:encode)
  WAx::WAxHTTPLibs::Builder.check_for_name_collision(String, "to_xs")
  WAx::WAxHTTPLibs::Builder.check_for_name_collision(Fixnum, "xchr")
end
end
end
######################################################################
...
if String.method_defined?(:encode)
  ...
  module Builder
    module XChar # :nodoc:
      CP1252_DIFFERENCES, UNICODE_EQUIVALENT = WAx::WAxHTTPLibs::Builder::XChar::CP1252.each.
        inject([[],[]]) {|(domain,range),(key,value)|
          [domain << key,range << value]
        }.map {|seq| seq.pack('U*').force_encoding('utf-8')}
  
      XML_PREDEFINED = Regexp.new('[' +
       WAx::WAxHTTPLibs::Builder::XChar::PREDEFINED.keys.pack('U*').force_encoding('utf-8') +
      ']')
  
      INVALID_XML_CHAR = Regexp.new('[^'+
       WAx::WAxHTTPLibs::Builder::XChar::VALID.map { |item|
          case item
          when Fixnum
  ...
  #
  class Fixnum
    XChar =WAx::WAxHTTPLibs::Builder::XChar if ! defined?(XChar)
  
    # XML escaped version of chr. When <tt>escape</tt> is set to false
    # the CP1252 fix is still applied but utf-8 characters are not
    # converted to character entities.
    def xchr(escape=true)
      n = WAx::WAxHTTPLibs::Builder::XChar::CP1252[self] || self
      case n when *XChar::VALID
        WAx::WAxHTTPLibs::Builder::XChar::PREDEFINED[n] or 
          (n<128 ? n.chr : (escape ? "&##{n};" : [n].pack('U*')))
      else
       WAx::WAxHTTPLibs::Builder::XChar::REPLACEMENT_CHAR
      end
    end
  end
